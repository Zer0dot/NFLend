...
describe("LoanManager persistent instance", function () {
    ...
    before(async function () {
        ...
        WETH = new ethers.Contract(WETH_ADDRESS, ERC20_ABI, accounts[0]);
    ...
    it("Account 1 should not be able to fulfill the request a second time", async function () {
        await expect(loanManager.connect(accounts[1]).fulfillRequest("2")).to.be.revertedWith("LoanManager: Fulfilled");
    });
    ...
    it("Should have accumulating debt", async function () {
        balanceBefore = await loanManager.getRequestDebtBalance("2");
        await sendDummyTx();
        balanceAfter = await loanManager.getRequestDebtBalance("2");
    ...
    it("Account 0 should repay borrow request with id \"2\" for 0.5 WETH", async function () {
        console.log("Old debt balance:", (await loanManager.getRequestDebtBalance("2")).toString());
        await expect(loanManager.repay(
            "2",
            ethers.utils.parseEther("0.5")
        )).to.not.be.reverted;
        console.log("New debt balance:", (await loanManager.getRequestDebtBalance("2")).toString());
    });
    ...
    it("Account 0 should repay the entirety of borrow request with id \"2\"", async function () {
        await expect(loanManager.repay(
            "2",
            MAX_UINT256
        )).to.not.be.reverted;
